...
/*************************************************
 * Name:        kyber_polyvec_compress
 *
 * Description: Compress and serialize vector of polynomials
 *
 * Arguments:   - unsigned char *r: pointer to output byte array
 *              - const kyber_polyvec *a: pointer to input vector of polynomials
 **************************************************/
void kyber_polyvec_compress (unsigned char *r, const kyber_polyvec *a) {
    ...
    for (i = 0; i < KYBER_K; i++) {
        for (j = 0; j < KYBER_N / 8; j++) {
            for (k = 0; k < 8; k++)
                t[k] =
                ((((uint32_t)freeze16 (a->vec[i].coeffs[8 * j + k]) << 11) + KYBER_Q / 2) / KYBER_Q) &
                0x7ff;
...
/*************************************************
 * Name:        kyber_polyvec_decompress
 *
 * Description: De-serialize and decompress vector of polynomials;
 *              approximate inverse of kyber_polyvec_compress
 *
 * Arguments:   - kyber_polyvec *r:       pointer to output vector of polynomials
...
void kyber_polyvec_decompress (kyber_polyvec *r, const unsigned char *a) {
    ...
    for (i = 0; i < KYBER_K; i++) {
        for (j = 0; j < KYBER_N / 8; j++) {
            ...
            (((a[11 * j + 0] | (((uint32_t)a[11 * j + 1] & 0x07) << 8)) * KYBER_Q) + 1024) >> 11;
            r->vec[i].coeffs[8 * j + 1] =
            ((((a[11 * j + 1] >> 3) | (((uint32_t)a[11 * j + 2] & 0x3f) << 5)) * KYBER_Q) +
             1024) >>
            11;
            ...
            ((((a[11 * j + 4] >> 1) | (((uint32_t)a[11 * j + 5] & 0x0f) << 7)) * KYBER_Q) +
             1024) >>
            11;
            r->vec[i].coeffs[8 * j + 4] =
            ((((a[11 * j + 5] >> 4) | (((uint32_t)a[11 * j + 6] & 0x7f) << 4)) * KYBER_Q) +
             1024) >>
            11;
            ...
            ((((a[11 * j + 8] >> 2) | (((uint32_t)a[11 * j + 9] & 0x1f) << 6)) * KYBER_Q) +
             1024) >>
            11;
            r->vec[i].coeffs[8 * j + 7] =
            ((((a[11 * j + 9] >> 5) | (((uint32_t)a[11 * j + 10] & 0xff) << 3)) * KYBER_Q) +
             1024) >>
            11;
        }
        a += 352;
...
void kyber_polyvec_compress (unsigned char *r, const kyber_polyvec *a) {
    ...
    for (i = 0; i < KYBER_K; i++) {
        for (j = 0; j < KYBER_N / 4; j++) {
            for (k = 0; k < 4; k++)
                t[k] =
                ((((uint32_t)freeze16 (a->vec[i].coeffs[4 * j + k]) << 10) + KYBER_Q / 2) / KYBER_Q) &
                0x3ff;
...
void kyber_polyvec_decompress (kyber_polyvec *r, const unsigned char *a) {
    ...
    for (i = 0; i < KYBER_K; i++) {
        for (j = 0; j < KYBER_N / 4; j++) {
            ...
            (((a[5 * j + 0] | (((uint32_t)a[5 * j + 1] & 0x03) << 8)) * KYBER_Q) + 512) >> 10;
...
void kyber_polyvec_compress (unsigned char *r, const kyber_polyvec *a) {
    ...
    for (i = 0; i < KYBER_K; i++) {
        for (j = 0; j < KYBER_N / 8; j++) {
            for (k = 0; k < 8; k++)
                t[k] =
                ((((uint32_t)freeze16 (a->vec[i].coeffs[8 * j + k]) << 9) + KYBER_Q / 2) / KYBER_Q) &
                0x1ff;
...
void kyber_polyvec_decompress (kyber_polyvec *r, const unsigned char *a) {
    ...
    for (i = 0; i < KYBER_K; i++) {
        for (j = 0; j < KYBER_N / 8; j++) {
            r->vec[i].coeffs[8 * j + 0] =
            (((a[9 * j + 0] | (((uint32_t)a[9 * j + 1] & 0x01) << 8)) * KYBER_Q) + 256) >> 9;
            r->vec[i].coeffs[8 * j + 1] =
            ((((a[9 * j + 1] >> 1) | (((uint32_t)a[9 * j + 2] & 0x03) << 7)) * KYBER_Q) + 256) >> 9;
            r->vec[i].coeffs[8 * j + 2] =
            ((((a[9 * j + 2] >> 2) | (((uint32_t)a[9 * j + 3] & 0x07) << 6)) * KYBER_Q) + 256) >> 9;
            r->vec[i].coeffs[8 * j + 3] =
            ((((a[9 * j + 3] >> 3) | (((uint32_t)a[9 * j + 4] & 0x0f) << 5)) * KYBER_Q) + 256) >> 9;
            r->vec[i].coeffs[8 * j + 4] =
            ((((a[9 * j + 4] >> 4) | (((uint32_t)a[9 * j + 5] & 0x1f) << 4)) * KYBER_Q) + 256) >> 9;
            r->vec[i].coeffs[8 * j + 5] =
            ((((a[9 * j + 5] >> 5) | (((uint32_t)a[9 * j + 6] & 0x3f) << 3)) * KYBER_Q) + 256) >> 9;
            r->vec[i].coeffs[8 * j + 6] =
            ((((a[9 * j + 6] >> 6) | (((uint32_t)a[9 * j + 7] & 0x7f) << 2)) * KYBER_Q) + 256) >> 9;
            r->vec[i].coeffs[8 * j + 7] =
            ((((a[9 * j + 7] >> 7) | (((uint32_t)a[9 * j + 8] & 0xff) << 1)) * KYBER_Q) + 256) >> 9;
        }
        a += 288;
    }
}
...
void kyber_polyvec_compress (unsigned char *r, const kyber_polyvec *a) {
    ...
    for (i = 0; i < KYBER_K; i++) {
        for (j = 0; j < KYBER_N; j++) {
            r[j] =
            ((((uint32_t)freeze16 (a->vec[i].coeffs[j]) << 8) + KYBER_Q / 2) / KYBER_Q) & 0xff;
        }
        r += 256;
    }
}
...
#else
#error "Unsupported compression of kyber_polyvec"
#endif

/*************************************************
 * Name:        kyber_polyvec_tobytes
 *
...
/*************************************************
 * Name:        kyber_polyvec_frombytes
 *
 * Description: De-serialize vector of polynomials;
 *              inverse of kyber_polyvec_tobytes
 *
 * Arguments:   - unsigned char *r: pointer to output byte array
 *              - const kyber_polyvec *a: pointer to input vector of polynomials
 **************************************************/
...
/*************************************************
 * Name:        kyber_polyvec_ntt
 *
 * Description: Apply forward NTT to all elements of a vector of polynomials
 *
 * Arguments:   - kyber_polyvec *r: pointer to in/output vector of polynomials
 **************************************************/
void kyber_polyvec_ntt (kyber_polyvec *r) {
    int i;
    for (i = 0; i < KYBER_K; i++) kyber_poly_ntt (&r->vec[i]);
}

/*************************************************
 * Name:        kyber_polyvec_invntt
 *
...
void kyber_polyvec_invntt (kyber_polyvec *r) {
    int i;
    for (i = 0; i < KYBER_K; i++) kyber_poly_invntt (&r->vec[i]);
}

/*************************************************
 * Name:        kyber_polyvec_pointwise_acc
 *
 * Description: Pointwise multiply elements of a and b and accumulate into r
 *
 * Arguments: - poly *r:          pointer to output polynomial
 *            - const kyber_polyvec *a: pointer to first input vector of polynomials
 *            - const kyber_polyvec *b: pointer to second input vector of polynomials
 **************************************************/
