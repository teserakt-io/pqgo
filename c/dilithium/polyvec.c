...
void polyvecl_freeze (polyvecl *v) {
    ...
    for (i = 0; i < L; ++i) poly_freeze (v->vec + i);
}
...
int polyvecl_chknorm (const polyvecl *v, uint32_t bound) {
    ...
    for (i = 0; i < L; ++i) ret |= poly_chknorm (v->vec + i, bound);
...
void polyveck_csubq (polyveck *v) {
    ...
    for (i = 0; i < K; ++i) poly_csubq (v->vec + i);
}
...
void polyveck_freeze (polyveck *v) {
    ...
    for (i = 0; i < K; ++i) poly_freeze (v->vec + i);
}
...
void polyveck_shiftl (polyveck *v, unsigned int k) {
    ...
    for (i = 0; i < K; ++i) poly_shiftl (v->vec + i, k);
}
...
void polyveck_invntt_montgomery (polyveck *v) {
    ...
    for (i = 0; i < K; ++i) poly_invntt_montgomery (v->vec + i);
}
...
int polyveck_chknorm (const polyveck *v, uint32_t bound) {
    ...
    for (i = 0; i < K; ++i) ret |= poly_chknorm (v->vec + i, bound);
...
void polyveck_use_hint (polyveck *w, const polyveck *u, const polyveck *h) {
    ...
    for (i = 0; i < K; ++i) poly_use_hint (w->vec + i, u->vec + i, h->vec + i);
}
